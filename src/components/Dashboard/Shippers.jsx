...
function Shippers() {
	...
	const formik = useFormik({
		...
		validationSchema: Yup.object({
			...
			phone_number: Yup.string().max(11, "Must be 11 characters").required("phone number is required"),
		}),
		...
		<div className="shippers">
			<Box component="div" sx={{ my: 3 }}>
				<Accordion>
					...
					<AccordionDetails>
						<Box component="form" onSubmit={formik.handleSubmit} className="add-shipper-form">
							<TextField
								id="shippername"
								label="Shipper Name"
								variant="outlined"
			...
			<Grid container spacing={3}>
				{searchOptions.map((shipper) => (
					<Grid item md={6} xs={12} lg={4} key={shipper.email}>
						<Shipper shipper={shipper} removeShipper={deleteShipper} openNotification={openNotification} />
					</Grid>
				))}
			</Grid>
